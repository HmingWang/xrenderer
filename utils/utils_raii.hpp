...
allocateDeviceMemory(vk::raii::Device const &device,
...
template <typename T>
void copyToDevice(vk::raii::DeviceMemory const &deviceMemory, T const *pData,
...
struct BufferData {
  ...
  void upload(vk::raii::PhysicalDevice const &physicalDevice,
    ...
    vk::raii::su::BufferData stagingBuffer(
        physicalDevice, device, dataSize,
  ...
  // the DeviceMemory should be destroyed before the Buffer it is bound to; to
  // get that order with the standard destructor of the BufferData, the order of
  // DeviceMemory and Buffer here matters
...
struct ImageData {
  ...
  // the DeviceMemory should be destroyed before the Image it is bound to; to
  // get that order with the standard destructor of the ImageData, the order of
  // DeviceMemory and Image here matters
  vk::Format format;
  vk::raii::DeviceMemory deviceMemory = nullptr;
  ...
  SwapChainData(vk::raii::PhysicalDevice const &physicalDevice,
    ...
    if (surfaceCapabilities.currentExtent.width ==
      ...
      swapchainExtent.width =
          vk::su::clamp(extent.width, surfaceCapabilities.minImageExtent.width,
    ...
    vk::SwapchainCreateInfoKHR swapChainCreateInfo(
        {}, surface,
        vk::su::clampSurfaceImageCount(3u, surfaceCapabilities.minImageCount,
...
struct TextureData {
  ...
  void setImage(vk::raii::CommandBuffer const &commandBuffer,
    ...
    void *data =
        needsStaging
            ...
            : imageData.deviceMemory.mapMemory(
                  0, imageData.image.getMemoryRequirements().size);
    ...
    if (needsStaging) {
      ...
      vk::raii::su::setImageLayout(
          commandBuffer, imageData.image, imageData.format,
      ...
      vk::raii::su::setImageLayout(commandBuffer, imageData.image,
                                   imageData.format,
                                   vk::ImageLayout::eTransferDstOptimal,
...
std::pair<uint32_t, uint32_t> findGraphicsAndPresentQueueFamilyIndex(
  ...
  uint32_t graphicsQueueFamilyIndex =
      vk::su::findGraphicsQueueFamilyIndex(queueFamilyProperties);
...
makeCommandBuffer(vk::raii::Device const &device,
                  vk::raii::CommandPool const &commandPool) {
...
makeDescriptorPool(vk::raii::Device const &device,
...
vk::raii::DescriptorSetLayout makeDescriptorSetLayout(
  ...
  for (size_t i = 0; i < bindingData.size(); i++) {
    bindings[i] = vk::DescriptorSetLayoutBinding(
        vk::su::checked_cast<uint32_t>(i), std::get<0>(bindingData[i]),
...
vk::raii::Device
makeDevice(vk::raii::PhysicalDevice const &physicalDevice,
...
void updateDescriptorSets(
    ...
    std::vector<std::tuple<vk::DescriptorType, vk::raii::Buffer const &,
                           vk::DeviceSize, vk::raii::BufferView const *>> const
        &bufferData,
    vk::raii::su::TextureData const &textureData, uint32_t bindingOffset = 0) {
...
void updateDescriptorSets(
  ...
  if (!textureData.empty()) {
    ...
    writeDescriptorSets.emplace_back(
        ...
        vk::DescriptorType::eCombinedImageSampler, imageInfos.data(), nullptr,
        nullptr);
  }
